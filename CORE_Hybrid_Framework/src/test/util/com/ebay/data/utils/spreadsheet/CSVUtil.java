...
public class CSVUtil {
  ...
  /**
   * Read data from csv formatted file. Put the excel sheet in the same folder
   ...
   * @throws Exception
   */
  public static Iterator<Object[]> getDataFromCSVFile(Class<?> clazz,
      String filename, String[] fields, EasyFilter filter, boolean readHeaders) {
    return getDataFromCSVFile(clazz, filename, fields, filter, readHeaders,
        null);
  }
    ...
    try {
      ...
      for (int i = 0; i < csvData[0].length; i++) {
        if (testTitleColumnIndex == -1
            && TestObject.TEST_TITLE.equalsIgnoreCase(csvData[0][i])) {
          testTitleColumnIndex = i;
        } else if (testSiteColumnIndex == -1
            && TestObject.TEST_SITE.equalsIgnoreCase(csvData[0][i])) {
      ...
      // Let's check for blank rows first. The first row is the header
      StringBuffer sbBlank = new StringBuffer();
      for (int i = 1; i < csvData.length; i++) {
        if (testTitleColumnIndex != -1
            && testSiteColumnIndex != -1
            && (csvData[i][testTitleColumnIndex].trim().length() == 0 || csvData[i][testSiteColumnIndex]
                .trim().length() == 0)) {
      ...
      for (int i = 1; i < csvData.length; i++) {
          ...
          if (uniqueDataSet.contains(uniqueString)) {
            throw new Exception(
                "Duplicate TestTitle and Site combination found in the spreadsheet "
                    + "with TestTitle = {" + csvData[i][testTitleColumnIndex]
                    + "} " + "and Site = {" + csvData[i][testSiteColumnIndex]
                    + "}");
          }
        ...
        if (fields == null) {
          for (int j = 0; j < csvData[0].length; j++) {
            // Fix for null values not getting created when number
            // of columns in a row is less than expected.
      ...
      if ((!readHeaders && sheetData.isEmpty())
        ...
        Console.log(LogType.WARNING, "No matching data found on csv file: "
   ...
   * 
   * @param clazz
   *          - null means use the absolute file path, otherwise use relative
   *          path under the class
   * @param filename
   * @param delimiter
   *          - null means ","
   * @return
   */
  public static ArrayList<String> getHeaderFromCSVFile(Class<?> clazz,
   ...
   * @throws IOException
   */
  public static String[][] read(InputStream is, String delim)
      throws IOException {
